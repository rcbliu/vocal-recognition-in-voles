...
%                this may lead to overwlapping distributions. Default:
%                1:nData
...
def.hist = 1; %%megan addition. The old version made the filled plot by making a histogram.
    %%I want to try just the outline
...
if ~isempty(varargin) && ~ischar(varargin{1}) && ~isstruct(varargin{1})
    ...
    parserObj.addOptional('hist',def.hist);
...
for iData = goodData'
    ...
    %%this was originally just 'switch opt.xyOri' through the corresponding
    %%'end'
    if opt.hist == 1
        switch opt.xyOri
            case 'normal'
        ...
        if sum(xArray(1,:))/iData == length(xArray(1,:))
            hh(iData)= plot(xArray(2,:),yHist(1,:),'color',opt.color{iData},'linew',3);
            plot(xArray(1,:),yHist(1,:),'color',[.7 .7 .7], 'linew', 1);
        else
            hh(iData)= plot(xArray(1,:),yHist(1,:),'color',opt.color{iData},'linew',3);
            plot(xArray(2,:),yHist(1,:),'color',[.7 .7 .7], 'linew', 1);
        end
    end
    ...
    if opt.showMM > 0
        ...
        if opt.showMM == 7
            ...
            md(iData,1,1) = nanmean(currentData);
            md(iData,2,1) = nanmean(currentData)+nanstd(currentData);
            md(iData,3,1) = nanmean(currentData)-nanstd(currentData);
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues+xOffset,m,'+r','Color','k','MarkerSize',12);
            end
            ...
            if opt.showMM == 8
                if strcmp(opt.histOri, 'center')
                    x_vals = opt.xValues+xOffset;
                else
                    x_vals = opt.xValues+xOffset/4;
                end
%                 line(repmat(x_vals,2,1),md(:,2:3)', 'color', 'k', 'linew', 4);
%                 scatter(x_vals, md(:,1),40,'s','filled','MarkerFaceColor',...
%                     'w', 'MarkerEdgeColor', 'k', 'LineWidth', 0.25);
                    try
                        line(repmat(x_vals,2,1),md(:,2:3)', 'color', 'k', 'linew', 8);
                        scatter(x_vals, md(:,1),150,'s','filled','MarkerFaceColor',...
                            'w', 'MarkerEdgeColor', 'k', 'LineWidth', 1);
                    catch
                        line(repmat(x_vals',2,1),md(:,2:3)', 'color', 'k', 'linew', 8);
                        scatter(x_vals, md(:,1),150,'s','filled','MarkerFaceColor',...
                            'w', 'MarkerEdgeColor', 'k', 'LineWidth', 1);
...
if opt.addSpread
    if isHistogram
        ...
        try
            sh = plotSpread(ah,data,'xValues',opt.xValues+xOffset,'xyOri',opt.xyOri);
            set(sh{1}(ishandle(sh{1})),'color',opt.color{1,1}./255);
        catch me
            if strcmp(me.identifier,'MATLAB:UndefinedFunction')
                error('plotSpread not found. Please download it from the Matlab File Exchange')
            else
